...
# Add the directory path to Python's import path
analysis_dir = r'C:\Users\James\Desktop\sliceAnalysis-master\ceed-1.0.0.dev2\ceed\analysis'
sys.path.append(analysis_dir)

from ceed.analysis import CeedDataReader
from pprint import pprint

ceed_data = 'C:\quetzalcoatl\workspace\James_q\Experiments/20240604slice1Cont_merged.h5'
...
shapes = sorted(reader.shapes_intensity_rendered.keys())
print('Found the following shapes in the experiment: {}'.format(shapes))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[0], reader.shapes_intensity_rendered[shapes[0]].shape))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[1], reader.shapes_intensity_rendered[shapes[1]].shape))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[2], reader.shapes_intensity_rendered[shapes[2]].shape))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[3], reader.shapes_intensity_rendered[shapes[3]].shape))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[4], reader.shapes_intensity_rendered[shapes[4]].shape))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[5], reader.shapes_intensity_rendered[shapes[5]].shape))
print('The intensity array for shape "{}" is size {}'.format(
    shapes[6], reader.shapes_intensity_rendered[shapes[6]].shape))
